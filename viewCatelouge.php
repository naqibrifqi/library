    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row pad-botm">
                <div class="col-md-12">
                    <p><a href="catelouge.php">Catelogue</a> >
                        <?php
                        $sql = "SELECT BookName,id FROM tblbooks WHERE id = " . $_GET['id'];
                        $query = $dbh->prepare($sql);
                        $query->bindParam(':bookid', $bookid, PDO::PARAM_STR);
                        if ($query->execute()) {
                            $results = $query->fetchAll(PDO::FETCH_OBJ);
                            foreach ($results as $result) { 
                                echo '<a href="#">' . $result->BookName . '</a>';
                            }
                        }
                        ?>
                    </p>
                                        ...
                                        <center>
                                            <table class="table table-bordered table-hover">
                                                <td width="350"> <img src="admin/uploads/<?php echo htmlentities($result->bookcover) ?>" height="440" width="400">
                                                    <br>
                                                    <center>
                                                        <p><?php echo htmlentities($result->quantity); ?> Books Available</p>
                                                    </center>
                                                </td>
                                                <td>
                                                    <h5> <b>Title: <?php echo htmlentities($result->BookName); ?></b></h5>
